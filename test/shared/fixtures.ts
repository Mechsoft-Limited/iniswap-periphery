...
export async function v2Fixture(provider: Web3Provider, [wallet]: Wallet[]): Promise<V2Fixture> {
  ...
  // deploy V2
  const factoryV2 = await deployContract(wallet, IniFactory, [wallet.address])

  // deploy routers
  const router01 = await deployContract(wallet, IniRouter01, [factoryV2.address, WETH.address], overrides)
  const router02 = await deployContract(wallet, IniRouter02, [factoryV2.address, WETH.address], overrides)

  // event emitter for testing
  const routerEventEmitter = await deployContract(wallet, RouterEventEmitter, [])

  // deploy migrator
  const migrator = await deployContract(wallet, IniMigrator, [factoryV1.address, router01.address], overrides)
  ...
  const WETHPair = new Contract(WETHPairAddress, JSON.stringify(IIniPair.abi), provider).connect(wallet)
